...
createProfileCard({
	{
		...
		getData = function(profile)
			...
			for i = 1, #licenses do
				licenseLabels[#licenseLabels + 1] = ("%s Délivré le %s (%s)"):format(licenses[i].label,
					licenses[i].issued, licenses[i].confiscated == "true" and 'Confisqué' or 'Actif')
			end
	...
	},
	{
		...
		icon = 'gavel',
		getData = function(profile)
			local charges = MySQL.rawExecute.await(
			'SELECT `charge` AS label, SUM(`count`) AS count FROM `ox_mdt_reports_charges` WHERE `charge` IS NOT NULL AND `stateId` = ? GROUP BY `charge`',
				{ profile.stateId }) or {}
			local chargeLabels = {}
